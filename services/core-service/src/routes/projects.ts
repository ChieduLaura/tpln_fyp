...
async function ensureSoftDeleteColumn(): Promise<void> {
...
router.post("/projects", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO projects (org_id, name, description, status, start_date, end_date, created_by)
       VALUES ($1, $2, $3, COALESCE($4, 'active'), $5, $6, $7)
       RETURNING id, org_id, name, description, status, start_date, end_date, created_by, created_at, updated_at`,
      [org_id, name, description ?? null, status ?? null, start_date ?? null, end_date ?? null, req.user.id]
    );
...
router.get("/projects", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT DISTINCT p.id, p.org_id, p.name, p.description, p.status, p.start_date, p.end_date, p.created_by, p.created_at, p.updated_at
       FROM projects p
       JOIN organizations o ON o.id = p.org_id
...
router.get("/projects/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const projectResult = await pool.query(
      `SELECT id, org_id, name, description, status, start_date, end_date, created_by, created_at, updated_at
       FROM projects
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE projects
       ...
       WHERE id = $1 AND deleted_at IS NULL
       RETURNING id, org_id, name, description, status, start_date, end_date, created_by, created_at, updated_at`,
      [projectId, name ?? null, description ?? null, status ?? null, start_date ?? null, end_date ?? null]
    );
